...
import { getInactiveMembersReport, exportInactiveMembersReport } from '../api/reports'
import toast from 'react-hot-toast'

export default function InactiveMembersReport() {
  const [page, setPage] = useState(1)
  const [hasSearched, setHasSearched] = useState(false)

  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['inactive-members-report', page],
    queryFn: () => getInactiveMembersReport({
      page,
      limit: 20
    }),
    enabled: hasSearched
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportInactiveMembersReport({})
      const blob = new Blob([response.data], { type: 'text/csv' })
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `inactive-members-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Inactive Members Report</span>
        </nav>
      </div>

      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900">Inactive Members Report</h1>
      </div>
