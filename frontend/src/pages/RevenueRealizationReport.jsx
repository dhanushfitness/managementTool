...
import { getRevenueRealizationReport, exportRevenueRealizationReport } from '../api/reports'
import api from '../api/axios'
import toast from 'react-hot-toast'

export default function RevenueRealizationReport() {
  ...
  // Fetch Revenue Realization report data
  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['revenue-realization-report', filters, page],
    queryFn: () => getRevenueRealizationReport({
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportRevenueRealizationReport({
        fromDate: filters.fromDate,
        toDate: filters.toDate,
        billNumber: filters.billNumber || undefined,
        salesRepId: filters.salesRepId !== 'all' ? filters.salesRepId : undefined,
      ...
      link.href = url
      link.download = `revenue-realization-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden" style={{ boxSizing: 'border-box' }}>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 w-full max-w-full overflow-hidden">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Revenue Realization</span>
        </nav>
      </div>

      {/* Page Title */}
      <div className="text-center w-full max-w-full overflow-hidden">
        <h1 className="text-3xl font-bold text-gray-900">Revenue Realization</h1>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 w-full max-w-full overflow-hidden">
        {isLoading ? (
          <LoadingPage message="Loading Revenue Realization report..." fullScreen={false} />
        ) : !hasSearched ? (
