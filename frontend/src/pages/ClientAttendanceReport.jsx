...
export default function ClientAttendanceReport() {
  ...
  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['client-attendance-report', filters, page],
    queryFn: () => getClientAttendanceReport({
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportClientAttendanceReport({
        fromDate: filters.fromDate,
        toDate: filters.toDate,
        serviceId: filters.serviceId !== 'all' ? filters.serviceId : undefined,
        gender: filters.gender !== 'all' ? filters.gender : undefined
      ...
      link.href = url
      link.download = `client-attendance-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Attendance Report</span>
        </nav>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div>
            ...
            <select
              value={filters.serviceVariationId}
              onChange={(e) => handleFilterChange('serviceVariationId', e.target.value)}
              className="w-40 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
            >
              <option value="all">All</option>
            </select>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="border border-gray-300 px-4 py-2 text-left">Total Check-Ins</th>
                      <th className="border border-gray-300 px-4 py-2 text-left">Last Check-In</th>
                    </tr>
                  </thead>
                  <tbody>
                    {records.map((record, index) => (
                      <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="border border-gray-300 px-4 py-2">{record.totalCheckIns}</td>
                        <td className="border border-gray-300 px-4 py-2">{formatDate(record.lastCheckIn)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
