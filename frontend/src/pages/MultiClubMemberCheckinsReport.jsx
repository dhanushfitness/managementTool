...
import { getMultiClubMemberCheckinsReport, exportMultiClubMemberCheckinsReport } from '../api/reports'
import toast from 'react-hot-toast'

export default function MultiClubMemberCheckinsReport() {
  ...
  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['multiclub-member-checkins-report', filters, page],
    queryFn: () => getMultiClubMemberCheckinsReport({
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportMultiClubMemberCheckinsReport({
        dateRange: filters.dateRange,
        search: filters.search || undefined
      })
      ...
      link.href = url
      link.download = `multiclub-member-checkins-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">MultiClub Member Check-Ins</span>
        </nav>
      </div>

      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900">MultiClub Member Check-Ins</h1>
      </div>
                ...
                {records.length === 0 ? (
                  ...
                  records.map((record, index) => (
                    <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{formatDateTime(record.checkInTime)}</td>
                      <td className="border border-gray-300 px-4 py-2">{formatDateTime(record.checkOutTime)}</td>
                      <td className="border border-gray-300 px-4 py-2">{record.method}</td>
                    </tr>
                  ))
                )}
              </tbody>
