...
export default function SuspensionsReport() {
  ...
  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['suspensions-report', filters, page],
    queryFn: () => getSuspensionsReport({
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportSuspensionsReport({
        fromDate: filters.fromDate,
        toDate: filters.toDate,
        search: filters.search || undefined
      })
      ...
      link.href = url
      link.download = `suspensions-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Suspensions</span>
        </nav>
      </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="border border-gray-300 px-4 py-2 text-left">Suspension Duration</th>
                ...
                {records.length === 0 ? (
                  ...
                  records.map((record, index) => (
                    <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{record.suspensionDuration}</td>
