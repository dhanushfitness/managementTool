...
import { getMembershipReport, exportMembershipReport } from '../api/reports'
...
export default function MembershipReport() {
  ...
  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['membership-report', filters, page],
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportMembershipReport({
        fromDate: filters.fromDate,
        toDate: filters.toDate
      })
      ...
      link.href = url
      link.download = `membership-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Membership Report</span>
        </nav>
      </div>

      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900">Membership Report</h1>
      </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="border border-gray-300 px-4 py-2 text-left">Bill No</th>
                  ...
                  <th className="border border-gray-300 px-4 py-2 text-right">Bill Amount</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">Pay Mode</th>
                </tr>
              </thead>
              <tbody>
                {records.length === 0 ? (
                  ...
                  records.map((record, index) => (
                    <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{record.billNo}</td>
                      <td className="border border-gray-300 px-4 py-2">{formatDate(record.startDate)}</td>
                      <td className="border border-gray-300 px-4 py-2">{formatDate(record.endDate)}</td>
                      ...
                      <td className="border border-gray-300 px-4 py-2 text-right">{record.billAmount}</td>
                      <td className="border border-gray-300 px-4 py-2">{record.payMode}</td>
                    </tr>
                  ))
                )}
              </tbody>
