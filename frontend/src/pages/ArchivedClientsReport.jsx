...
import { getArchivedClientsReport, exportArchivedClientsReport } from '../api/reports'
import toast from 'react-hot-toast'

export default function ArchivedClientsReport() {
  const [page, setPage] = useState(1)
  const [hasSearched, setHasSearched] = useState(false)

  const { data: reportData, isLoading, refetch } = useQuery({
    queryKey: ['archived-clients-report', page],
    queryFn: () => getArchivedClientsReport({
      page,
      limit: 20
    }),
    enabled: hasSearched
  ...
  const handleExportExcel = async () => {
    try {
      const response = await exportArchivedClientsReport({})
      const blob = new Blob([response.data], { type: 'text/csv' })
      const url = window.URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = `archived-clients-report-${new Date().toISOString().split('T')[0]}.csv`
    ...
    <div className="space-y-6 max-w-full w-full overflow-x-hidden">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <nav className="text-sm">
          ...
          <span className="text-gray-400 mx-2">/</span>
          <span className="text-orange-600 font-medium">Archived Clients</span>
        </nav>
      </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="border border-gray-300 px-4 py-2 text-left">Last Service Date</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">Archived Date</th>
                ...
                {records.length === 0 ? (
                  ...
                  records.map((record, index) => (
                    <tr key={record._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="border border-gray-300 px-4 py-2">{formatDate(record.lastServiceDate)}</td>
                      <td className="border border-gray-300 px-4 py-2">{formatDate(record.archivedDate)}</td>
